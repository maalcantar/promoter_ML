...
def create_promoter_df_for_ml(positive_promoter_df,
    ...
    inputs
    positive_promoter_df: pandas dataframe
        dataframe containing positive promoter sequences
    negative_permuted_promoter_df: pandas dataframe
        dataframe containing negative promoters (obtained by permuting positive promoters)
    random_df: pandas dataframe
        dataframe with random negative promoter sequences
    random_idx_start: integer
        where to start indexing negative random num_sequences_to_fill

    taken out: percentage_permuted: percent of permuted promoters that should constitute the negative subset

    returns
    promoters_for_ML_df: pandas dataframe
        dataframe containing promoters ready for ML (see function description for more detail)
    random_idx_start: integer
        value indicating the index for the next available random sequence
...
def concat_intragenic_region(prom_for_ML, organism_name):

    """
    concatenate intragenic regions with positive and negative promoter sets for specified organism_motifs_dict

    inputs
    prom_for_ML: pandas dataframe
        dataframe with positive and negative promoter num_sequences
    organism_name: character string
        character string indicating organism whose intragenic regions
        are being concatenated. Currently supported inputs are: human, athaliana, ecoli

    returns
    prom_for_ML: pandas dataframe
        dataframe with new intragenic regions concatenated to positive
        and negative promoter sequences
    """

    # load in gene transcripts
    ...
    # determine number of sequences we need to concatenate (should equal number)
    # of positive sequences
    prom_df = prom_for_ML.copy()[(prom_for_ML['organism']==organism_name_in_df) &
                                    (prom_for_ML['class']==1)]
    prom_seq_num = prom_df.copy()[(prom_df['organism'] == organism_name_in_df)].shape[0]
    num_intra_seqs = intragenic_seq_df.shape[0]

    # randomly sampling from gene transcripts
...
def main():
    ...
    EPDnew_promoters_for_ML_df = concat_intragenic_region(EPDnew_promoters_for_ML_df,
                                                          organism_name='human')
    EPDnew_promoters_for_ML_df = concat_intragenic_region(
    ...
    RegulonDB_promoters_for_ML_df = concat_intragenic_region(
